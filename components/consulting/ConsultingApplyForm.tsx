...
/**
 * 유료 1:1 입시 전략 컨설팅 신청 폼.
 *
 * 전화번호 포맷·검증·개인정보 동의·상태 처리 패턴은
 * components/winter/ConsultForm.tsx를 따르되, 상품이 달라(무료 상담 vs 유료
 * 컨설팅) 컴포넌트는 공유하지 않는다 — 수집 항목·개인정보 목적·학년 선택지가
 * 다르고, 억지로 합치면 양쪽이 서로를 깨뜨린다.
 *
 * 현재 결제 시스템은 없다 — 신청을 먼저 받고 일정·결제 방법은 이후 안내한다.
 * 결제가 붙으면 성공 화면(SubmittedView)만 결제 안내로 바꾸면 된다.
 */
...
export default function ConsultingApplyForm({
  source,
}: {
  ...
  /** 폼에 처음 손을 댄 시점을 한 번만 기록한다 */
  const markStarted = () => {
    if (startedRef.current) return;
    startedRef.current = true;
    trackConsulting("consulting_form_start", { source });
  };
  ...
  const handleSubmit = async () => {
    ...
    trackConsulting("consulting_form_submit", { source });
    try {
      ...
      if (!res.ok) throw new Error("consulting request failed");
      trackConsulting("consulting_form_success", { source });
      setStatus("done");
    } catch {
      trackConsulting("consulting_form_error", { source });
      setStatus("error");
    }
  };
  ...
  if (status === "done") {
    return (
      ...
      >
        ...
        <p className="mt-3 text-sm leading-relaxed text-white/60 break-keep md:text-base">
          상담 가능한 일정과 결제 방법을 확인해 연락드리겠습니다.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      id="apply-form"
      className="scroll-mt-24 rounded-2xl border border-accent/40 bg-white/[0.03] p-6 text-left md:p-10"
    >
      <p className="text-[11px] uppercase tracking-[0.25em] text-accent">
        Apply
      </p>
      <h3 className="mt-2 text-xl font-bold text-white md:text-2xl">
        {CONSULTING_INFO.name} 신청
      </h3>
      <p className="mt-2 text-sm text-white/50 break-keep">
        {CONSULTING_INFO.priceLabel} / {CONSULTING_INFO.priceUnit} · 신청을
        남겨주시면 상담 가능한 일정과 결제 방법을 안내해 드립니다.
      </p>
      ...
      <div className="mt-7 space-y-5">
        ...
        <div>
          ...
          >
            {CONSULTING_GRADE_OPTIONS.map((g) => (
              <button
                key={g}
                type="button"
                role="radio"
                ...
                className={`min-h-[44px] rounded-xl border px-2 py-3 text-sm font-medium transition-colors ${
                  grade === g
        ...
        <div>
          ...
          <input
            ...
            aria-describedby={
              showPhoneError ? "consulting-phone-error" : undefined
            }
            className={`${INPUT_CLASS} ${
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => {
              markStarted();
              setConcern(e.target.value.slice(0, CONSULTING_CONCERN_MAX));
            }}
            rows={3}
            maxLength={CONSULTING_CONCERN_MAX}
            placeholder={
              "예: 성적으로 어디까지 가능한지 궁금해요 /\n국민대가 목표인데 무엇을 올려야 할지 모르겠어요 /\n실기를 지금 바꿔야 할지 고민돼요"
            }
            className={`${INPUT_CLASS} resize-none leading-relaxed`}
          />
          <p className="mt-1.5 text-right text-[11px] text-white/35">
            {concern.length}/{CONSULTING_CONCERN_MAX}
          </p>
        </div>
        ...
        <div>
          ...
          {showPrivacy && (
            <div className="mt-3 space-y-1.5 rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3.5 text-xs leading-relaxed text-white/50">
              <p>
                · 수집 항목: 학생 이름, 학년, 연락처, 희망 대학(선택), 고민
                내용(선택)
              </p>
              <p>
                · 이용 목적: 1:1 미대 입시 전략 컨설팅 신청 확인 및 상담 연락
                (그 외 목적으로 사용하지 않습니다)
              </p>
              <p>
                · 보유 기간: [상담 완료 후 지체 없이 파기]
                {/* TODO: 원장님 확인 — winter ConsultForm과 동일하게 정책 확정 후 갱신 */}
              </p>
              <p>· 동의를 거부할 수 있으나, 거부 시 컨설팅 신청이 제한됩니다.</p>
            </div>
          )}
        </div>
        ...
        <button
          ...
          disabled={!canSubmit}
          className="w-full rounded-xl bg-accent py-4 text-base font-bold text-black transition-opacity hover:opacity-85 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
        >
          {status === "submitting" ? (
        ...
        {status === "error" && (
          <p className="text-center text-sm text-red-400 break-keep">
            일시적인 오류로 접수하지 못했습니다. 잠시 후 다시 시도하시거나 아래
            톡톡으로 문의해 주세요.
          </p>
        )}

        <p className="text-center text-xs text-white/40 break-keep">
          결제는 신청 단계에서 진행되지 않습니다 — 일정 확인 후 안내해
          드립니다.
        </p>
      </div>
    </div>
